...
class NativeIncrementalCompilationIT : KGPBaseTest() {
    ...
    @DisplayName("Adding external dependency doesn't force IC to rebuild (as no actual dependency has been added)")
    @GradleTest
    fun externalDependency(gradleVersion: GradleVersion) {
        nativeProject("native-incremental-simple", gradleVersion) {
            var modified = 0L
            val mainKtCache = getFileCache("native-incremental-simple", "src/hostMain/kotlin/main.kt", "")
            build(
                "linkDebugExecutableHost",
                buildOptions = buildOptions.copy(
                    nativeOptions = BuildOptions.NativeOptions(
                ...
                modified = mainKtCache.toFile().lastModified()
            }
            ...
            build(
                ...
                assertDirectoryExists(mainKtCache)
                assertEquals(modified, mainKtCache.toFile().lastModified())
            }
        }
    }
